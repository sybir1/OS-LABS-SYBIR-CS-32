...
void signal_handler(int sig) {
    if (sig == SIGUSR1) {
        ...
        printf("Програма 1 (батько): Сума: %d\n", sum);
    }
}
...
void run_child_process() {
    ...
    execlp("./childsProgram", "childsProgram", shm_id_str, NULL);
    perror("execlp");
    exit(1);
}
...
void handle_parent_process() {
    while (1) {
        int n, i, input;
        printf("Програма 1 (батько): Введіть кількість чисел для сумування (0 - вихід): ");
        scanf("%d", &n);
        if (n <= 0) {
            break;
        }
        for (i = 0; i < n; i++) {
            printf("Програма 1 (батько): Введіть число: ");
            scanf("%d", &input);
